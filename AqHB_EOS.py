...
#    def solute_activity(self, comp, x, compobjs):
#        gamma = 2.*molality.*gamma_P1;
#%     %     gamma = 1;
#    activity = log(molality) + gamma;
#                 MW_H20 = datatable{'H20','MW'}/1000;
#%     molality = x_gas./(MW_H20*x_w);
#
#    molality = x(strcmp(comprequest,comps))/(MW_H20*x_w);

#    # Output of symbolic integration for v_ast_P
#    def solutevol(self, comp, P, T):
#        omega = comp.AqHB['omega_born']
#        v1 = comp.AqHB['v']['v1']
#        v2 = comp.AqHB['v']['v2']
#        v3 = comp.AqHB['v']['v3']
...
#
#        volume = (
#            (omega*tau - T*omega + omega*theta - P*s10*v3
#             - s10*v4*np.log(P + psi) - P**2*s20*v3 - P**3*s30*v3
#             - P*s20*v4*np.log(P + psi) - T*s10*v2*np.log(P + psi)
#             - T*s11*v4*np.log(P + psi) + s10*tau*v2*np.log(P + psi)
#             + s10*theta*v2*np.log(P + psi) - P*T**2*s11*v1
#             - P*T**3*s12*v1 - P*T**2*s12*v3 - P**2*T*s20*v1
#             - P**2*T*s21*v3 - P**3*T*s30*v1 - P**3*T*s31*v3
#             + P**2*s20*tau*v1 + P**3*s30*tau*v1 + P**2*s20*theta*v1
#             + P**3*s30*theta*v1 - P**2*s30*v4*np.log(P + psi)
#             - T**2*s11*v2*np.log(P + psi) - T**3*s12*v2*np.log(P + psi)
#             - T**2*s12*v4*np.log(P + psi) - P**2*T**2*s21*v1
#             - P**2*T**3*s22*v1 - P**2*T**2*s22*v3
#             - P**3*T**2*s31*v1 - P**3*T**3*s32*v1
#             - P**3*T**2*s32*v3 - P*T*s10*v1 - P*T*s11*v3
#             + P*s10*tau*v1 + P*s10*theta*v1
#             + P**2*T**2*s22*tau*v1 + P**3*T**2*s32*tau*v1
#             + P**2*T**2*s22*theta*v1 + P**3*T**2*s32*theta*v1
#             - P**2*T**2*s31*v2*np.log(P + psi)
#             - P**2*T**3*s32*v2*np.log(P + psi)
#             - P**2*T**2*s32*v4*np.log(P + psi)
#             + P*T*s11*tau*v1 + P*T*s11*theta*v1
#             - P*T*s20*v2*np.log(P + psi)
#             - P*T*s21*v4*np.log(P + psi)
#             + P*s20*tau*v2*np.log(P + psi)
#             + P*s20*theta*v2*np.log(P + psi)
#             + T*s11*tau*v2*np.log(P + psi)
#             + T*s11*theta*v2*np.log(P + psi)
#             + P*T**2*s12*tau*v1 + P**2*T*s21*tau*v1
#             + P**3*T*s31*tau*v1 + P*T**2*s12*theta*v1
#             + P**2*T*s21*theta*v1 + P**3*T*s31*theta*v1
#             - P*T**2*s21*v2*np.log(P + psi)
#             - P*T**3*s22*v2*np.log(P + psi)
#             - P*T**2*s22*v4*np.log(P + psi)
#             - P**2*T*s30*v2*np.log(P + psi)
#             - P**2*T*s31*v4*np.log(P + psi)
#             + P**2*s30*tau*v2*np.log(P + psi)
#             + P**2*s30*theta*v2*np.log(P + psi)
#             + T**2*s12*tau*v2*np.log(P + psi)
#             + T**2*s12*theta*v2*np.log(P + psi)
#             + P*T**2*s22*tau*v2*np.log(P + psi)
#             + P**2*T*s31*tau*v2*np.log(P + psi)
#             + P*T**2*s22*theta*v2*np.log(P + psi)
#             + P**2*T*s31*theta*v2*np.log(P + psi)
#             + P**2*T**2*s32*tau*v2*np.log(P + psi)
#             + P**2*T**2*s32*theta*v2*np.log(P + psi)
#             + P*T*s21*tau*v2*np.log(P + psi)
#             + P*T*s21*theta*v2*np.log(P + psi)
#             ) /
#            (R*T*(tau - T + theta)
#             * (s32*P**2*T**2 + s31*P**2*T + s30*P**2 + s22*P*T**2
#                + s21*P*T + s20*P + s12*T**2 + s11*T + s10)
#             )
#        )
#        return volume
